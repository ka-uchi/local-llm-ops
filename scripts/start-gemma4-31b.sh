...
LOG_FILE="${LOG_DIR}/gemma4-31b.log"
...
require_cuda_visible_devices "1"
...
if [[ -f "${PID_FILE}" ]]; then
  ...
  if [[ -n "${existing_pid}" ]] && kill -0 "${existing_pid}" 2>/dev/null; then
    fail "gemma4-31b is already running with PID ${existing_pid}"
  fi
  rm -f "${PID_FILE}"
fi
